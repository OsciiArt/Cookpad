...
def TTA(model, data):
    ...
    for flip, flop, rot90 in zip([0,1], [0,1], [0,1]):
        gen = test_generator(data, "/home/akiyama/PycharmProjects/Cookpad/input/test/",
...
def train(epochs, seed):
    ...
    num_fold = 5
    save_path = base_name
    load_model = "train_180308_1_Dense_4gen_ts0859"
    ...
    preds = []
    for i in range(1):
        weight_path = "/home/akiyama/PycharmProjects/Cookpad/model/{}_fold{}_stage5.hdf5".format(load_model, i)
        model.load_weights(filepath=weight_path)

        # Getting Test prediction
        pred = TTA(model, X_test)
        preds.append(pred)
    preds = np.mean(preds, axis=0)

    # save
    submission = pd.DataFrame({'id': X_test, 'predict': np.argmax(preds, axis=1)})
    submit_path = "output/submission" + save_path + "_0.tsv"
