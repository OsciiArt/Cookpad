...
def test_generator(x_train, img_dir, batch_size, shuffle, flip, flop):
    # x_train = x_train.as_matrix()
    # y_train = y_train.as_matrix()
    batch_index = 0
    n = x_train.shape[0]
...
def margin_generator(x_train, img_dir, batch_size, shuffle, flip, flop):
    # x_train = x_train.as_matrix()
    # y_train = y_train.as_matrix()
    batch_index = 0
    n = x_train.shape[0]
    ...
    while 1:
        ...
        for id in batch_id:
            ...
            img = cv2.imread(img_dir + '/{}'.format(x_train[id]))
            longer = np.max(img.shape[:2])
            img_new = np.zeros([longer, longer, 3], np.uint8)
            img_new[(longer - img.shape[0])//2:(longer - img.shape[0])//2 + img.shape[0],
                    (longer - img.shape[1]) // 2:(longer - img.shape[1])//2 + img.shape[1],] = img
            # print("img shape", img.shape)
            img = cv2.resize(img_new, (input_size, input_size))
            if flip:
                img = img[:,::-1]
            if flop:
                img = img[::-1]
...
def crop_generator(x_train, img_dir, batch_size, shuffle, flip, flop):
    # x_train = x_train.as_matrix()
    # y_train = y_train.as_matrix()
    batch_index = 0
    n = x_train.shape[0]
    ...
    while 1:
        ...
        for id in batch_id:
            ...
            img = img[(img.shape[0] - shorter)//2:(img.shape[0] - shorter)//2+shorter,
                      (img.shape[1] - shorter) // 2:(img.shape[1] - shorter)//2+shorter]
            img = cv2.resize(img, (input_size, input_size))
            if flip:
                img = img[:,::-1]
            if flop:
...
def train(epochs, seed):
    ...
    gen_tst_pred = margin_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=False, flop=False)
    pred_test5 = model.predict_generator(generator=gen_tst_pred,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred = margin_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=True, flop=False)
    pred_test6 = model.predict_generator(generator=gen_tst_pred2,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred = margin_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=True, flop=True)
    pred_test7 = model.predict_generator(generator=gen_tst_pred3,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred = margin_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=False, flop=False)
    pred_test8 = model.predict_generator(generator=gen_tst_pred4,
                                        steps=np.ceil(X_test.shape[0]/batch_size))


    gen_tst_pred = crop_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=False, flop=False)
    pred_test9 = model.predict_generator(generator=gen_tst_pred,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred = crop_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=True, flop=False)
    pred_test10 = model.predict_generator(generator=gen_tst_pred2,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred = crop_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=True, flop=True)
    pred_test11 = model.predict_generator(generator=gen_tst_pred3,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred = crop_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=False, flop=False)
    pred_test12 = model.predict_generator(generator=gen_tst_pred4,
                                        steps=np.ceil(X_test.shape[0]/batch_size))

    pred_test = (pred_test1 + pred_test2 + pred_test3 + pred_test4
                 + pred_test5 + pred_test6 + pred_test7 + pred_test8
                 + pred_test9 + pred_test10 + pred_test11 + pred_test12

                 ) / 12
    submission = pd.DataFrame({'id': X_test, 'predict': np.argmax(pred_test, axis=1)})
    submit_path = "output/submission" + save_path + "testaugx12_val_loss" + str(score[0]) + "_val_acc" + str(score[1]) + ".tsv"
