...
def train(epochs, seed):
    ...
    model.load_weights(filepath="model/train_180210_1_Dense_5th_training_ts0418.hdf5")
    ...
    pred_test1 = model.predict_generator(generator=gen_tst_pred,
                                        steps=np.ceil(X_holdout.shape[0]/batch_size))
    gen_tst_pred2 = test_generator(X_holdout, "input/train", batch_size, shuffle=False,
                                   flip=True, flop=False)
    pred_test2 = model.predict_generator(generator=gen_tst_pred2,
                                        steps=np.ceil(X_holdout.shape[0]/batch_size))
    gen_tst_pred3 = test_generator(X_holdout, "input/train", batch_size, shuffle=False,
                                   flip=True, flop=True)
    pred_test3 = model.predict_generator(generator=gen_tst_pred3,
                                        steps=np.ceil(X_holdout.shape[0]/batch_size))
    gen_tst_pred4 = test_generator(X_holdout, "input/train", batch_size, shuffle=False,
                                   flip=False, flop=False)
    ...
    pred_class_sorted = np.argsort(-pred_test, axis=1)[:, :5]
    print(pred_class_sorted.shape)
    pred_proba_sorted = np.sort(-pred_test, axis=1)[:, :5]
    ...
    submit_path = "output/submission" + save_path + "valid_augx8_val_loss" + str(score[0]) + "_val_acc" + str(score[1]) + ".csv"
