...
def TTA(model, data):
    preds = []
    batch_size = 64
    for flip, flop, rot90 in zip([0,1], [0,1], [0,1]):
        gen = test_generator(data, "input/test", batch_size, shuffle=False,
                             flip=flip, flop=flop, rot90=rot90)
        pred = model.predict_generator(generator=gen,
                                       steps=np.ceil(data.shape[0]/batch_size))
        preds.append(pred)
...
def train(epochs, seed):
    ...
    model.load_weights(filepath="model/train_180215_1_Dense_6th_training_ts2220.hdf5")
