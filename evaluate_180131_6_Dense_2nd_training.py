...
def train(epochs, seed):
    ...
    model.load_weights(filepath="model/train_180131_6_Dense_2nd_training_ts2059_seed0.hdf5")
    ...
    gen_tst_pred = test_generator(X_test[:,:,::-1], "input/test", batch_size, shuffle=False)
    pred_test2 = model.predict_generator(generator=gen_tst_pred,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    pred_test = (pred_test1 + pred_test2) / 2
    submission = pd.DataFrame({'id': X_test, 'predict': np.argmax(pred_test, axis=1)})
    submit_path = "output/submission" + save_path + "testaugx2_val_loss" + str(score[0]) + "_val_acc" + str(score[1]) + ".tsv"
