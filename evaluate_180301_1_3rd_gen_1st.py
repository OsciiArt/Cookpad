...
def train(epochs, seed):
    ...
    gen_tst_pred = test_generator(X_test, "/home/akiyama/PycharmProjects/Cookpad/input/test/", batch_size, shuffle=False,
                                   flip=False, flop=False)
    pred_test1 = model2.predict_generator(generator=gen_tst_pred,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred2 = test_generator(X_test, "/home/akiyama/PycharmProjects/Cookpad/input/test/", batch_size, shuffle=False,
                                   flip=True, flop=False)
    pred_test2 = model2.predict_generator(generator=gen_tst_pred2,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred3 = test_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=True, flop=True)
    pred_test3 = model2.predict_generator(generator=gen_tst_pred3,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred4 = test_generator(X_test, "/home/akiyama/PycharmProjects/Cookpad/input/test/", batch_size, shuffle=False,
                                   flip=False, flop=False)
    pred_test4 = model2.predict_generator(generator=gen_tst_pred4,
                                        steps=np.ceil(X_test.shape[0]/batch_size))

    gen_tst_pred = test_generator(X_test, "/home/akiyama/PycharmProjects/Cookpad/input/test/", batch_size, shuffle=False,
                                   flip=False, flop=False, rot90=True)
    pred_test5 = model2.predict_generator(generator=gen_tst_pred,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred2 = test_generator(X_test, "/home/akiyama/PycharmProjects/Cookpad/input/test/", batch_size, shuffle=False,
                                   flip=True, flop=False, rot90=True)
    pred_test6 = model2.predict_generator(generator=gen_tst_pred2,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred3 = test_generator(X_test, "/home/akiyama/PycharmProjects/Cookpad/input/test/", batch_size, shuffle=False,
                                   flip=True, flop=True, rot90=True)
    pred_test7 = model2.predict_generator(generator=gen_tst_pred3,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    gen_tst_pred4 = test_generator(X_test, "/home/akiyama/PycharmProjects/Cookpad/input/test/", batch_size, shuffle=False,
                                   flip=False, flop=False, rot90=True)
    pred_test8 = model2.predict_generator(generator=gen_tst_pred4,
                                        steps=np.ceil(X_test.shape[0]/batch_size))
    pred_test = (pred_test1 + pred_test2 + pred_test3 + pred_test4
                 + pred_test5 + pred_test6+ pred_test7 + pred_test8) / 8
    submission = pd.DataFrame({'id': X_test, 'predict': np.argmax(pred_test, axis=1)})
    submit_path = "output/submission" + save_path + "testaugx8rot90_val_loss" + str(score[0]) + "_val_acc" + str(score[1]) + ".tsv"
