...
def main():
    ...
    cv = []
    for i in range(fold):
        _, _, submit_path, valid_score =train(epochs=200, seed=i)
    ...
    for i in range(fold):
        ...
        prediction = pd.read_csv(submit_path).iloc[:,1].as_matrix()
        prediction = prediction.reshape([-1,1])
        prediction_list.append(prediction)

    predictions = np.concatenate(prediction_list, axis=1)
    predictions = np.mean(predictions, axis=1)
    test_id = pd.read_csv(submit_path).iloc[:,0].as_matrix()
    submission = pd.DataFrame({'id': test_id, 'predict': predictions})
    cv_mean = np.array(cv).mean()
    file_name = os.path.basename(os.path.splitext(__file__)[0])
    submission.to_csv("output/submission_"+ file_name + "_cv_score_" + str(cv_mean) + "_ensamble.csv", index=False, header=False)
