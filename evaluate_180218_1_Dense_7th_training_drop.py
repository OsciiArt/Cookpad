...
def train(epochs, seed):
    ...
    def prediction(f, batchgenerator):
        num_step = int(np.ceil(X_test.shape[0] / batch_size))
        pred_mean = np.zeros([X_test.shape[0], num_class], np.float32)
        pred_var = np.zeros([X_test.shape[0], num_class], np.float32)
    ...
    pred_mean6, pred_var6 = prediction(f, gen_tst_pred2)

    gen_tst_pred3 = test_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=True, flop=True, rot90=True)
    pred_mean7, pred_var7 = prediction(f, gen_tst_pred3)

    gen_tst_pred4 = test_generator(X_test, "input/test", batch_size, shuffle=False,
                                   flip=False, flop=False, rot90=True)
    pred_mean8, pred_var8 = prediction(f, gen_tst_pred4)

    pred_test = (pred_mean1 + pred_mean2 + pred_mean3 + pred_mean4
                 + pred_mean5 + pred_mean6+ pred_mean7 + pred_mean8) / 8
    submission = pd.DataFrame({'id': X_test, 'predict': np.argmax(pred_test, axis=1)})
    submit_path = "output/submission" + save_path + "testaugx8rot90_val_loss" + str(score[0]) + "_val_acc" + str(score[1]) + "_drop0.1.tsv"
